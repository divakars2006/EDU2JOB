...
const LoginPage = () => {
  ...
  const loginWithGoogle = useGoogleLogin({
    onSuccess: async (codeResponse) => {
      ...
      setError('');
      try {
        // Here we send the token to our backend
        // Note: The googleLogin API service function might need to be adjusted 
        // to handle access_token instead of credential if previously using <GoogleLogin />
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0f] flex flex-col items-center justify-center p-4 font-sans tracking-wide">
      ...
      <div className="w-full max-w-md pt-8 pb-8 pl-5 pr-12 rounded-2xl border border-white/10 bg-white/[0.02] backdrop-blur-md shadow-2xl">
        <h2 className="text-2xl font-bold text-white text-center mb-2">Welcome <span className="text-purple-600">Back</span></h2>
        <p className="text-sm text-gray-400 text-center mb-8">Please sign in to your account</p>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col w-full gap-5">
          ...
          <div className="w-full flex flex-col gap-1">
            <label className="text-sm font-medium text-gray-300 text-left" htmlFor="email">Email Address</label>
          ...
          <div className="flex items-center gap-2 mt-1">
            <input
              type="checkbox"
              id="remember"
              className="w-4 h-4 rounded border-white/10 bg-black/20 text-purple-600 focus:ring-purple-500 focus:ring-offset-gray-900"
              disabled={isLoading}
            />
